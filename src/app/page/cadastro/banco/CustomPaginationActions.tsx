...
export default function CustomPaginationActions(data: any) {
  ...
  const [data1, setData1] = React.useState(data.data.banks);
  const [data2, setData2] = React.useState(data.data2.banks);
  ...
  const fetchData2 = async () => {
    const response = await fetch(
      "https://erp.sitesdahora.com.br/api/banks-inativo",
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
  ...
  const inativar = { status_bank: "INATIVO" };
  const ativar = { status_bank: "ATIVO" };

  const buscaFiltrada1 = useMemo(() => {
    const lowerBusca = busca.toLowerCase();
    return data1.filter((data1: any) =>
      data1.name_bank.toLowerCase().includes(lowerBusca)
    );
  }, [busca, data1]);
  ...
  const buscaFiltrada2 = useMemo(() => {
    ...
    return data2.filter((data2: any) =>
      data2.name_bank.toLowerCase().includes(lowerBusca)
    );
  }, [busca, data2]);
  ...
  async function Salvar(form: FormData) {
    ...
    if (banco === "") {
      const result = async () => {
        const response = await fetch(
          "https://erp.sitesdahora.com.br/api/bank-create",
          {
            method: "POST",
      ...
      const result = async () => {
        const response = await fetch(
          `https://erp.sitesdahora.com.br/api/bank-edit/${banco.id}`,
          {
            method: "PUT",
            body: JSON.stringify(data),
            headers: {
                            ...
                            <TableCell style={{ width: 160 }} align="right">
                              ...
                              <Tooltip title="INABILITAR">
                                <Button
                                  onClick={async () => {
                                    const response = await fetch(
                                      `https://erp.sitesdahora.com.br/api/bank-status/${row.id}`,
                                      {
                                        cache: "no-cache",
                                        method: "PUT",
